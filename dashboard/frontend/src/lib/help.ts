...
export const HELP: Record<string, HelpEntry> = {
  ...
  "/kg": {
    ...
    what: "Product-level overview of the knowledge graph — per domain: code entities, requirement docs, links, and coverage %. Drill into a domain for detail.",
    persona: "Everyone (devs consume KG context while building)",
    prerequisites: ["A domain ingested into the KG"],
    cli: ["keel kg context"],
  },
  "/kg/graph": {
    title: "Graph",
    what: "One home for both graph views: the ingested Knowledge Graph (code entities ↔ requirements, Neo4j) and a repo's structural Code Graph (graphify). Toggle between them at the top.",
    persona: "Leads / devs for review",
    prerequisites: ["KG view: Neo4j configured + domain ingested/linked", "Code view: a repo onboarded with Graphify"],
    cli: ["keel kg visualize", "keel code onboard --graphify"],
    tips: ["Deep-link the code view with ?view=code (e.g. from the Repository page)."],
  },
  "/kg/onboard": {
    title: "KG Onboarding",
  ...
  "/code-graph": {
    title: "Graph (Code view)",
    what: "Review a repo's graphify structural graph — validate what was captured before it feeds the KG. Now part of the unified Graph page (Knowledge → Graph); toggle to the Knowledge Graph there.",
