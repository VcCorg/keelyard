...
class BasicAgent(BaseAgent):
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt = f"You are a helpful Basic Agent.\n\nHistory:\n{history}\n\nUser: {message}\n\nAssistant:"
        try:
...
async def answer(input_text: str) -> str:
    """Run BasicAgent on a single input and return its response text."""
    global _eval_agent
    if _eval_agent is None:
        try:
            from config import Settings
            _eval_agent = BasicAgent(settings=Settings())
        except Exception:
            _eval_agent = BasicAgent()
