...
def validate_neo4j_connection(skip_check: bool = False) -> bool:
    ...
    from dva_agentic_cli.kg.validation import (
...
def init(
    ...
    if provider == "neo4j":
        ...
        from dva_agentic_cli.kg.validation import check_neo4j_availability
        ...
        from dva_agentic_cli.kg.lightrag_client import check_lightrag_availability
        ...
        with console.status(f"[bold green]Ingesting data into LightRAG from {resolved_source}..."):
            try:
                ...
                if resolved_format == "git":
                    from dva_agentic_cli.kg.parsers import parse_git_repository
...
def ingest_list(
    ...
    from dva_agentic_cli.kg.async_ingest import get_manager, JobStatus
...
def sync(
    ...
    """
    from dva_agentic_cli.kg.config import KGConfig
    from dva_agentic_cli.kg.sync import sync_to_lightrag
...
def stats(
    ...
    try:
        if config.provider == "neo4j":
            from dva_agentic_cli.kg.stats import get_stats
...
def visualize(
    ...
    from dva_agentic_cli.kg.visualize import create_visualization
